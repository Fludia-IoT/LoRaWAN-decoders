...
function decode_T1_MME(payload,type,recvTime){
  ...
  for(var i=15;i>=0;i--){
    if(recvTime != null){
      var d = new Date(recvTime)
      d.setSeconds(d.getSeconds() - ((15-i)*data.time_step*60));
      if((i!=15 && data.increments[i][1] == null) || data.index == null) data.indexes.unshift([d.toISOString(),null])
...
function decode_T1_MECA(payload,recvTime){
  ...
  for(var i=0;i<20;i++){
    if(recvTime != null){
      ...
      data.powers.push([d.toISOString(),(payload[5+2*i] & 0xFF) << 8 | (payload[6+2*i] & 0xFF)])
    }else{
