...
function decode_T1_MME(payload, type, recvTime){
  ...
  for(var i=8;i>=0;i--){
    if(recvTime != null){
      var d = new Date(recvTime)
      d.setSeconds(d.getSeconds() - ((8-i)*data.time_step*60));
      if((i!=8 && data.increments[i][1] == null) || data.index == null) data.indexes.unshift([d.toISOString(),null])
...
function decode_T1_MECA(payload, time_step, recvTime){
  ...
  for(var i=0;i<8;i++){
    if(recvTime != null){
      ...
      data.increments.push([d.toISOString(),(payload[5+2*i] & 0xFF) << 8 | (payload[6+2*i] & 0xFF)])
